...
/**
 * The unified case trail: everything that's happened to one case, in
 * order — the original feedback, the AI/manual triage, every level it's
 * passed through, every comment, its linked playbook run, and its
 * resolution. This is the same detail page reused across personas; only
 * how you arrive at it (Case Management list vs. an Exec's exceptions
 * feed) differs.
 */
export default function BusinessCaseTrailClient({ caseId }: { caseId: string }) {
  ...
  function load() {
    setLoading(true);
    fetch(`/api/business/action-board/${caseId}`)
      .then(async (res) => {
  ...
  async function postComment() {
    ...
    setPosting(true);
    await fetch(`/api/business/action-board/${caseId}/comments`, {
      method: "POST",
  ...
  async function resolve() {
    setResolving(true);
    await fetch(`/api/business/action-board/${caseId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status: "resolved", resolutionNote: resolutionDraft.trim() }),
    });
    setResolving(false);
    load();
  }
  ...
  return (
    <div>
      <Link href="/business/cases" className="subtitle" style={{ display: "inline-block", marginBottom: 10 }}>
        ...
        {item.status === "resolved" ? (
          ...
          <>
            <div className="field">
              ...
              <textarea value={resolutionDraft} onChange={(e) => setResolutionDraft(e.target.value)} />
            </div>
            <button className="btn btn-dark btn-sm" disabled={resolving} onClick={resolve}>

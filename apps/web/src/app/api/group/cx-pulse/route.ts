...
export async function GET() {
  ...
  const orgScore = await CxPulseScore.findOne({ ownerType: "parentOrg", ownerId: session.org._id, product: "customer_experience" }).sort({ period: -1 });
  ...
  const businessScores = await Promise.all(
    businesses.map(async (b) => {
      const score = await CxPulseScore.findOne({ ownerType: "business", ownerId: b._id, product: "customer_experience" }).sort({ period: -1 });

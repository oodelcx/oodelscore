...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  const goal = await CxGoal.findOne({ _id: id, ownerType: "parentOrg", ownerId: session.org._id });
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  ...
  await connectToDatabase();
  const { id } = await params;
  const removed = await CxGoal.findOneAndDelete({ _id: id, ownerType: "parentOrg", ownerId: session.org._id });

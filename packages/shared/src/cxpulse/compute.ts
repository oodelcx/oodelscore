...
async function responseScore(businessIds: Types.ObjectId[], from: Date, to: Date): Promise<number> {
  if (businessIds.length === 0) return 0;
  const responses = await Response.find({ businessId: { $in: businessIds }, submittedAt: { $gte: from, $lte: to } });
...
async function ownershipScore(businessIds: Types.ObjectId[]): Promise<number> {
  if (businessIds.length === 0) return 0;
  const items = await ActionBoardItem.find({ businessId: { $in: businessIds } }).select("ownerId");
 ...
 */
async function cultureScore(parentOrgId: Types.ObjectId | null, businessIds: Types.ObjectId[], quarter: string): Promise<number> {
  let playbookRate: number | null = null;
  if (parentOrgId) {
    const playbooks = await Playbook.find({ parentOrgId }).select("usageCount");
    ...
    const recentResponses = await Response.find({ businessId: { $in: businessIds }, submittedAt: { $gte: from } }).select("answers");
...
async function outcomeScore(businessIds: Types.ObjectId[]): Promise<number> {
  if (businessIds.length === 0) return 0;
...
/**
 * Computes one owner's current CX Pulse score. Never call this from a page
 * request (spec Section 7) — it fans out across responses, action items,
 * playbooks, and decision log entries. Call only from the nightly job below.
 */
export async function computeCxPulseForOwner(ownerType: BillingOwnerType, ownerId: Types.ObjectId): Promise<CxPulseResult> {
  ...
  const dimensions: ICxPulseDimensions = {
    ...
    response: await responseScore(businessIds, from, now),
    ownership: await ownershipScore(businessIds),
    culture: await cultureScore(parentOrgIdForCulture, businessIds, currentQuarterLabel(now)),
    outcome: await outcomeScore(businessIds),
  };
...
export async function recomputeAllCxPulseScores(): Promise<void> {
  ...
  const businesses = await Business.find({ active: true }).select("_id");
  for (const business of businesses) {
    const result = await computeCxPulseForOwner("business", business._id);
    await CxPulseScore.findOneAndUpdate(
      { ownerType: "business", ownerId: business._id, period },
      { $set: result },
      { upsert: true }
    );
  }

  const parentOrgs = await ParentOrganization.find().select("_id");
